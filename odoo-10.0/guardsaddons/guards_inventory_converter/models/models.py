...
class GuardsStockConversions(models.Model):
  ...
  from_uom_id = fields.Many2one(comodel_name='guards.stock.uom', string='From UOM', help='From which UOM you are converting',required=True)
  to_uom_id = fields.Many2one(comodel_name='guards.stock.uom', string='To UOM', help='Should be the base uom',required=True)
...
class GuardsSaleProductInheritConverter(models.Model):
  ...
  def update_quantity(self):
    ...
    if not self.product_id.base_uom:
      ...
      return
    if self.product_id.base_uom.id == self.product_uom_id.id:
      self.quantity = self.input_quantity
      return
    self.quantity = self.product_id.get_converted_quantity(self.input_quantity, self.product_uom_id)
